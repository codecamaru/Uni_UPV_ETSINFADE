...
from perceptronk import perceptron; from confus import confus
from linmach import linmach

if len(sys.argv) != 5:
    print('Usage: %s <data> <alphas> <bs> <k>' % sys.argv[0])
    sys.exit(1)
...
ki = np.fromstring(sys.argv[4], sep = ' ') # k parámetro  
...
for a in alphas:
    for b in bs:
        w,E,k=perceptron(train,b,a,ki); rl=np.zeros((M,1));
        rl = np.zeros((M,1))
        for m in range(M):
