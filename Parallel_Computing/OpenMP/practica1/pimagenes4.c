...
#define IMAGEN_SALIDA "lenna-filp4.ppm" 
  ...
  for (p = 0; p < pasos; p++) {
    for (i = 0; i < n; i++) {
      for (j = 0; j < m; j++) {
        ...
        #pragma omp parallel for private(l,v) reduction(+:red,green,blue,tot)
...
double t = omp_get_wtime();
  ...
  t = omp_get_wtime()-t;
  printf("Tiempo de ejecución paralelización bucle l para %d hilos %f segundos\n",nh,t);
