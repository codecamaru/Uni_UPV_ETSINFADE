...
#define IMAGEN_SALIDA "lenna-filp2.ppm" 
           ...
           int n, int m)
{
  ...
  for (p = 0; p < pasos; p++) {
    for (i = 0; i < n; i++) {
    #pragma omp parallel for private(j,resultado,tot,k,l,v)
      for (j = 0; j < m; j++) {
        resultado.r = 0;
        resultado.g = 0;
        resultado.b = 0;
...
double t = omp_get_wtime();
  ...
  t = omp_get_wtime()-t;
  printf("Tiempo de ejecución paralelización bucle j para %d hilos %f segundos\n",nh,t);
