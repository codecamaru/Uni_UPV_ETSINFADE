...
double calcula_integral1(double a, double b, int n)
{
  ...
  /* la variable iteradora no puede ser compartida openmp
   permite que sin expresarlo directamente sea privada */
   for (i=0; i<n; i++) {
      s+=f(a+h*(i+0.5));
   }
...
double calcula_integral2(double a, double b, int n)
{
   ...
   h=(b-a)/n;
 #pragma omp paralell for reduction(+: s) private(x)
 //para que en x no haya condicion de carrera, que como se escribe puede haber
 //tienes que probarlo tambien con el secuencial y que te de lo mismo
   for (i=0; i<n; i++) {
      x=a;
      x+=h*(i+0.5);
