...
#define NUM_PASOS 5
#define DIST_RADIO 8
#define IMAGEN_ENTRADA "Lenna1k.ppm"
#define IMAGEN_SALIDA "lenna-filp3.ppm" 
  ...
  for (p = 0; p < pasos; p++) {
    for (i = 0; i < n; i++) {
      for (j = 0; j < m; j++) {
        ...
        int blue = 0;
        tot = 0;
        #pragma omp parallel for private(k,l,v) reduction(+:red,green,blue,tot)
...
double t = omp_get_wtime();
  ...
  t = omp_get_wtime()-t;
  printf("Tiempo de ejecución paralelización bucle k para %d hilos %f segundos\n",nh,t);
