...
// Realign the rows of image a, of width w and height h PROCESAMIENTO COMPLETO DE LA IMAGEN
void realign( int w,int h,Byte a[] ) {
  ...
  for ( y = 1 ; y < h ; y++ ) {
    ...
    for ( off = 1 ; off < w ; off++ ) {
      ...
      // Update minimum distance and corresponding best offset hasta el momento
      if ( d < dmin ) { dmin = d; bestoff = off; }
    }
    // cada línea y debería desplazarse voff[y] píxeles a la izquierda respecto a la línea y-1
    voff[y] = bestoff;
  }
