  ...
  #pragma omp parallel private(i, e) 
  {
    ...
    for ( i = id ; i < n && d < c ; i+= nh ) {
      ...
      if ( e >= 0 ) {
        #pragma omp atomic
          d += e;
         // yo creo que como siempre se va a tener que ejecutar una condición u otra, hacerlas todas atomic es menos eficiente 
...
void realign( int w,int h,Byte a[] ) {
  ...
  /*
  averigua cuántos píxeles habría que desplazar cada línea respecto a la línea anterior, 
  para lograr que la diferencia de color entre ambas líneas sea lo menor posible
  */
  for ( y = 1 ; y < h ; y++ ) {

    // Find offset of line y that produces the minimum distance between lines y and y-1
    /*
    calcular la diferencia respecto a la línea anterior (y-1) sin aplicar ningún 
    desplazamiento (o sea, un desplazamiento de cero píxeles)
    */
