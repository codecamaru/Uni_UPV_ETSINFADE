  ...
  if (me == 0) {
    ...
    for ( rows_done = 0 ; rows_done < h ; rows_done++ ) {
      ...
      MPI_Recv(B, w, MPI_BYTE, MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD,
               &status);
    /* inicia la recepcion no bloqueante de una fila calculada de cualquier proceso */
    MPI_Irecv(B, w, MPI_BYTE, MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD,
               &req);  
    // mientras no se ha recibido nada y siguiente fila < filas totales
    int flag = 0; // 0 si no se ha producido la petición de recepción del mensaje, 1 si se ha producido
    while(MPI_Test(&req, &flag, &status), !flag && next_row < h){
