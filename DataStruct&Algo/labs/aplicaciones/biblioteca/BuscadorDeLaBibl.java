...
public class BuscadorDeLaBibl {
    ...
    public ListaConPI<Termino> hapax() {
        /* COMPLETAR */
        ListaConPI<Termino> lista = new LEGListaConPI<Termino>(); // lista a devolver
        ListaConPI<Termino> listaClaves = index.claves(); // lista con las claves de la BD
        ListaConPI<BuscadorDeLaBibl.Posting> valor; // lista con loa titulos en los que aparece Termino
        // Si hay más de un Posting, hay más de una aparición y ya no es un hapax
        for(listaClaves.inicio(); !listaClaves.esFin(); listaClaves.siguiente()){
            Termino clave = listaClaves.recuperar(); // clave a buscar en mapa
            valor = index.recuperar(clave); 
            if(valor.talla() == 1){ lista.insertar(clave); } // añadimos hapax a lista
        }
        if(lista.talla() == 0){ return null; }
        return /*CORREGIR*/ lista;
    }
    public Termino masFrecuente(ListaConPI<Termino> it){
        it.inicio();
        Termino t = it.recuperar();
        int mayorLong = (index.recuperar(t)).length();
        for(;!it.esFin(); it.siguiente()){
            int i = index.recuperar(t)).length();
            if(i > mayorLong){mayorLong = i; }
        }
        return t;
    }
}    