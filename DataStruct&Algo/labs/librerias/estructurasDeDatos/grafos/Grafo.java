...
public abstract class Grafo {
    ...
    public Arista[] arbolRecubrimientoBFS() {
        ...
        if(this.ordenVisita != numVertices() - 1){ return null; }
        return res; 
    }
    // Recorrido BFS del vertice origen, que almacena en res
    // su resultado
    ...
    public Arista[] kruskal() {       
        ...
        ColaPrioridad<Arista> cP = new MonticuloBinarioR0<Arista>();
        UFSet cc = new ForestUFSet(this.numVertices());
        Arista [] res = new Arista[numVertices() - 1];
        int cardinalE = 0;
        for(int i = 0; i < numVertices(); i++){
            ListaConPI<Adyacente> l = adyacentesDe(i);
            for(l.inicio(); !l.esFin(); l.siguiente()){
                Adyacente x = l.recuperar();
                cP.insertar(new Arista(i, x.destino, x.peso));
            }
        }
        while(cardinalE < numVertices() - 1 && !cP.esVacia()){
            Arista aux = cP.eliminarMin();
            int r1 = cc.find(aux.origen);
            int r2 = cc.find(aux.destino);
            if(r1 != r2){
                cc.union(r1, r2);
                res[cardinalE++] = aux;
            }
        }
        if(cardinalE == numVertices() - 1) { return res; }
        return null;
    }
    public int esHojaEnMST(int v){
        int[] verticesGrafo = this.toArrayBFS();
        boolean encontrado = false;
        for(int i = 0 ; i < verticesGrafo.length && !encontrado; i++){
            if(verticesGrafo[i] == v){ encontrado = true; }
        }
        if(!encontrado){ return -2; } // no se encuentra el vertice en grafo
        else{
            if(this.kruskal() == null){ return -1; } // no existe MST
            else{
                ListaConPI<Adyacente> l = this.adyacentesDe(v);
                if(l.talla() > 0){ return 0; } // el vertice no es una hoja
                else{ return 1; } // el vertice es una hoja 
            }
        }
    }
}
