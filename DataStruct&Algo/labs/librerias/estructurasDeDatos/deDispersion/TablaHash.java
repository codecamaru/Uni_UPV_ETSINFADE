...
public class TablaHash<C, V> implements Map<C, V> {
    ...
    protected final void rehashing() {
        /* COMPLETAR */
        //copia del array actual para luego almacenar
        ListaConPI<EntradaHash<C, V>>[] aux = elArray; 
        // longitud numero primo doble capacidad
        elArray = new LEGListaConPI[siguientePrimo(elArray.length * 2)]; 
        for (int i = 0; i < elArray.length; i++) {
            // inicializamos lista vacía (cubetas)
            elArray[i] = new LEGListaConPI<EntradaHash<C, V>>(); 
        }
        for(int i = 0; i < aux.length; i++){
            for(aux[i].inicio(); !aux[i].esFin(); aux[i].siguiente()){
                EntradaHash<C,V> elemento = aux[i].recuperar();
                elArray[indiceHash(elemento.clave)].insertar(new EntradaHash<C,V>(elemento.clave, elemento.valor));
            }
        }
    }
    ...
    public final double desviacionTipica() {
        ...
        for (ListaConPI l : elArray){
             sumaCuadrados += Math.pow((l.talla() - FC), 2);
        }
        double desvTip = Math.sqrt(sumaCuadrados / elArray.length);
        return desvTip; 
    }
    ...
    public final double costeMLocalizar() {
        ...
        for(int i = 0; i < elArray.length; i++){
            colisiones += elArray[i].talla() * (elArray[i].talla() - 1); 
        }
        colisiones = colisiones / 2;
        coste = colisiones / this.talla();
        return coste;
    }
